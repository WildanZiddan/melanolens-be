# MelanomaLens: Model Integration Audit & Definitive Solution Guide

**Project:** MelanoLens (AI-Powered Melanoma Early Screening Platform)  
**Scope:** Deep Learning Model Integration, Clinical Semiology (ABCD Rule), Explainable AI (XAI), and Full-Stack Pipeline  
**Source Baseline:** [`run_pipeline.py`] & [`2_Melanoma_Inference_and_XAI_Visualization.ipynb`]
**Target Integration:** [`Backend/melanolens-be`](/Backend/melanolens-be) & [`Frontend/melanolens`](/Frontend/melanolens)

---

## 1. Executive Summary

During the migration of the Vision Transformer (ViT-B/16) model from the research and prototyping environment (`run_pipeline.py` & `.ipynb`) to the production web backend (`melanolens-be`), **critical regressions and omissions occurred**.

Most urgently:

1. **The clinical diagnosis is inverted 180 degrees**: Malignant melanoma tumors are diagnosed as benign moles, and benign moles are diagnosed as malignant melanoma.
2. **The Explainable AI (XAI) attention map is completely fabricated**: The true Grad-CAM implementation was replaced with a hardcoded static Gaussian circle placed in the image center.
3. **The clinical ABCD semiology and TDS calculation was 100% dropped**: The API computes none of the morphological parameters (Asymmetry, Border, Color, Diameter) advertised in the UI and README.
4. **Severe preprocessing domain shift**: The model was trained on DullRazor hair-removed, Shades-of-Grey color-corrected images with black borders, but the web service feeds raw images with neutral gray borders.

This document details every flaw identified and provides **definitive, production-ready drop-in code** to resolve all issues.

---

## 2. Detailed Audit of Problems Found

```
┌─────────────────────────────────────────────────────────────────────────────────────────────────┐
│                                    SUMMARY OF INTEGRATION FLAWS                                 │
├────┬─────────────────────────────┬──────────────────────────────────────────────────────────────┤
│ #  │ Flaw                        │ Consequence                                                  │
├────┼─────────────────────────────┼──────────────────────────────────────────────────────────────┤
│ 1  │ Class Label Inversion       │ Benign is reported as Malignant; Malignant as Benign.        │
│ 2  │ Synthetic XAI Deception     │ Real Grad-CAM replaced by static Gaussian circle in center. │
│ 3  │ Omission of ABCD / TDS      │ No clinical semiology metrics returned or displayed.         │
│ 4  │ Preprocessing Domain Shift  │ No DullRazor, no Shades of Grey; Gray padding vs Black.     │
│ 5  │ Missing Dependencies        │ opencv-python and grad-cam missing from requirements.txt.   │
│ 6  │ Dropped Consensus Engine    │ 4-model multi-architecture ensemble reduced to single model. │
│ 7  │ Base64 Database Bloat       │ Raw image payloads stored as huge base64 strings in DB rows. │
└────┴─────────────────────────────┴──────────────────────────────────────────────────────────────┘
```

---

### Problem 1: Fatal Class Label Inversion (Benign $\leftrightarrow$ Malignant)

#### Root Cause

In [`run_pipeline.py`], the dataset folders were named:

```python
CATEGORIES = ['malignant', 'benign']
```

When loaded by PyTorch's `datasets.ImageFolder`, folders are **always sorted alphabetically**:

- Index `0` $\rightarrow$ `'benign'` (starts with **b**)
- Index `1` $\rightarrow$ `'malignant'` (starts with **m**)

In the evaluation loop of [`run_pipeline.py`]:

```python
target_class = 1  # Malignant
probs = F.softmax(outputs, dim=1)[:, 1]  # Index 1 is Malignant probability
```

In the notebook [`2_Melanoma_Inference_and_XAI_Visualization.ipynb`]:
...
#### What Happened in `ai_service.py`

The backend developer guessed that alphabetical sorting was based on "Melanoma" vs "Nevus":

```python
# ai_service.py (Lines 44-59 & 235-243)
CLASS_LABELS = {
    0: {'label': 'Melanoma (Kanker Ganas)', 'risk_level': 'Tinggi', ...},
    1: {'label': 'Nevus / Tahi Lalat (Jinak)', 'risk_level': 'Rendah', ...}
}
...
...
**The entire diagnostic output of the website is reversed.** A patient with a dangerous malignant lesion receives a "Low Risk / Benign" report, while patients with benign moles receive alarming "Malignant / High Risk" warnings.

---

### Problem 2: Explainable AI (XAI) Is Completely Fabricated

#### Root Cause

In [`run_pipeline.py`] and the notebook, real Grad-CAM was designed specifically for the ViT architecture:
...
In [`ai_service.py`], Grad-CAM was discarded because `grad-cam` was missing from `requirements.txt`. Instead, a hardcoded radial formula was written:

```python
# ai_service.py (Lines 177-188)
h, w, _ = img_np.shape
y, x = np.ogrid[:h, :w]
...
The website draws a synthetic Gaussian circle in the center of the image frame. If a lesion is off-center or irregular, the heatmap continues to illuminate whatever is in the center of the image. This violates clinical transparency requirements.

---

### Problem 3: Omission of the Medical ABCD & TDS Semiology Engine

#### Root Cause

The notebook contains an automated clinical feature extractor:

- **Asymmetry ($A$)**: Inertia moment rotation + orthogonal axis mismatch ($0 \sim 2$, weight $\times 1.3$).
- **Border Irregularity ($B$)**: Compactness ($\frac{P^2}{4\pi A}$) + 8-sector radial deviation ($0 \sim 8$, weight $\times 0.1$).
- **Color Variegation ($C$)**: HSV classification for White, Black, Red, Blue-Gray, Dark Brown, Light Brown ($1 \sim 6$, weight $\times 0.5$).
- **Diameter ($D$)**: Minimum enclosing circle scaled to millimeters ($1 \sim 5$, weight $\times 0.5$).
- **Total Dermatoscopy Score (TDS)**: $\text{TDS} = 1.3A + 0.1B + 0.5C + 0.5D$.
- **Clinical-AI Concordance**: Evaluates agreement between TDS category and model prediction.

#### Consequence

The website frontend boasts "Standardisasi Klinis ABCDE", but the backend API (`/api/skrining/predict`) returns zero ABCD metrics, creating a complete disconnect between the system's claims and its actual implementation.

---

### Problem 4: Preprocessing Domain Shift

The ViT model checkpoint was trained on images processed with:

1. **DullRazor Hair Removal**: Morphological BlackHat filtering + Telea inpainting (`cv2.inpaint`).
2. **Shades of Grey Color Constancy**: Minkowski illuminant normalization ($p=6.0$).
3. **Aspect-Ratio Conscious Black Border Padding**: `cv2.copyMakeBorder(..., value=[0, 0, 0])`.

In [`ai_service.py`]:

- No hair removal is performed (hair artifacts remain on the lesion).
- No color constancy is applied (images are sensitive to ambient lighting and camera hardware differences).
- Images are padded with **gray `(128, 128, 128)`** instead of **black `[0, 0, 0]`**.

---

## 3. Definitive Code Solutions

### Step 1: Update Dependency Manifest

Update [`Backend/melanolens-be/requirements.txt`](melanolens-be/requirements.txt) to include OpenCV, Grad-CAM, and SciPy:
...
### Step 2: Complete Drop-In Replacement for `ai_service.py`

Replace the entire contents of [`melanolens-be/ai_service.py`] with the following tested code.

This implementation:

1. Fixes the class index mapping (`0` = Benign / Nevus, `1` = Malignant / Melanoma).
2. Implements real ViT Grad-CAM targeting `encoder.layers[-1].ln_1` with token reshape.
3. Implements DullRazor hair removal and Shades of Grey color constancy.
4. Implements the full clinical ABCD semiology and TDS calculation engine.
5. Returns a unified JSON payload with model probabilities, true heatmap overlay, ABCD breakdown, and concordance rating.
...
### Step 3: Update `main.py` Database & API Logic

In [`melanolens-be/main.py`]:

1. `/api/skrining/predict` automatically includes the new `abcd` field in the response.
2. In `models.py`, optionally add `scan_responGambar = Column(Text, nullable=True)` to store the Grad-CAM heatmap base64, while `scan_respon` stores both the AI diagnosis and the TDS score.

---

### Step 4: Update Frontend `scan/page.tsx` to Display the Clinical ABCD Metrics

In [`melanolens/src/app/(protected-pages)/home/scan/page.tsx`](melanolens/src/app/%28protected-pages%29/home/scan/page.tsx), update `AIResult` to receive and display the ABCD parameters:
...
Add an interactive **ABCD Clinical Parameter Card** right below the Grad-CAM visualization:
    ...
    <div className="mt-3 p-3 bg-slate-50 dark:bg-slate-800/40 rounded-xl border border-slate-200 dark:border-slate-700">
      ...
      <div className="grid grid-cols-2 gap-2 text-[11px] mb-2">
        <div className="bg-white dark:bg-slate-800 p-2 rounded-lg border border-slate-100 dark:border-slate-700">
          ...
          <span className="font-bold">{result.abcd.a_score} / 2</span>
        </div>
        <div className="bg-white dark:bg-slate-800 p-2 rounded-lg border border-slate-100 dark:border-slate-700">
          <span className="text-slate-400 block text-[9px]">
            B - Border Irreg. (0-8)
          </span>
          <span className="font-bold">{result.abcd.b_score} / 8</span>
        </div>
        <div className="bg-white dark:bg-slate-800 p-2 rounded-lg border border-slate-100 dark:border-slate-700">
          <span className="text-slate-400 block text-[9px]">
            C - Color Variegation
          </span>
          <span className="font-bold">{result.abcd.c_score} warna</span>
        </div>
        <div className="bg-white dark:bg-slate-800 p-2 rounded-lg border border-slate-100 dark:border-slate-700">
          <span className="text-slate-400 block text-[9px]">
            D - Diameter Est.
          </span>
          <span className="font-bold">{result.abcd.diameter_mm} mm</span>
        </div>
      </div>

      <div className="text-[10px] text-slate-500 space-y-1">
        <p>
          <strong>Warna Terdeteksi:</strong>{" "}
          {result.abcd.detected_colors.join(", ")}
        </p>
        <p>
          <strong>Kesesuaian AI-Klinis:</strong>{" "}
          <span className="text-primary font-medium">
            {result.abcd.concordance}
          </span>
        </p>
      </div>
    </div>
...
Execute the following verification script in the backend directory to confirm all fixes before deploying:

```bash
python -c "
import torch
from ai_service import load_ai_model, predict_lesion
import numpy as np, cv2

print('Testing model load...')
load_ai_model()

# Create synthetic 224x224 RGB lesion
test_img = np.full((300, 300, 3), 200, dtype=np.uint8)
cv2.circle(test_img, (150, 150), 60, (50, 40, 30), -1)
_, buf = cv2.imencode('.jpg', test_img)

print('Testing prediction...')
res = predict_lesion(buf.tobytes())
print('Status:', res['status'])
print('Label:', res['label'])
print('Confidence:', res['confidence'], '%')
print('ABCD TDS:', res['abcd']['tds'])
print('Heatmap generated (len):', len(res['heatmap_base64']))
assert len(res['heatmap_base64']) > 100, 'Grad-CAM heatmap failed!'
...
1. **Safety**: Reversing the inverted class indexes eliminates the hazard of false-negative cancer reports.
2. **Transparency**: True ViT Grad-CAM gives patients and clinicians actual diagnostic interpretability.
3. **Medical Fidelity**: Integrating the clinical ABCD semiology and TDS calculation fulfills the project's core feature set.
4. **Accuracy**: Aligning preprocessing (DullRazor, Shades of Grey, black padding) eliminates the visual domain shift between training and web inference.
